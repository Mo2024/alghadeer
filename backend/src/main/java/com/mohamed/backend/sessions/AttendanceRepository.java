...
public interface AttendanceRepository extends JpaRepository<Attendance, Integer> {
    ...
    @Query(value = """
            ...
            """, nativeQuery = true)
    List<MainTopicView> findStudentTopics(@Param("studentId") Integer studentId, @Param("isPresent") boolean isPresent, @Param("semesterId") Integer semesterId);
    ...
    @Query(value = """
            ...
            FROM attendance a
            inner join sessions s on s.id = a.session_id
            WHERE a.student_id = :studentId and s.semester_id = :semesterId
            GROUP BY student_id
            """, nativeQuery = true)
    Double getAttendancePercentageByStudentId(@Param("studentId") Integer studentId, @Param("semesterId") Integer semesterId);
}
