package com.mohamed.backend.sessions;

import com.mohamed.backend.classes.dto.AddSubTopicDto;
import com.mohamed.backend.classes.dto.AttendanceRequestDTO;
import com.mohamed.backend.classes.dto.GetAttendanceStatusDto;
import com.mohamed.backend.sessions.dto.SessionView;
import com.mohamed.backend.sessions.dto.SessionViewExtends;
...
public class SessionController {
    ...
    @PostMapping("/all/edit-attendance")
    @Operation(
            summary = "Edits attendance of students for a given session",
            description = "Only admins/instructors/staff who is assigned to the class or session is authorized to perform this request."
    )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Success request - Request executed successfully"),
    ...
    public ResponseEntity<?> editAttendance(@RequestBody AttendanceRequestDTO attendanceRequest) {
        try {
            log.info("executing method [attendanceService].[editAttendance]");
            GetAttendanceStatusDto response = attendanceService.editAttendance(attendanceRequest);
            log.info("[attendanceService].[editAttendance] executed successfully");
            logger.logJsonObject("Response for [editAttendance]:\n{}", response);
            return ResponseEntity.ok().body(response);
        } catch (HandledRejection e) {
            return ResponseEntity
                    .badRequest()
    ...
    @PostMapping("/all/cancel-sessions-by-session-id")
    @Operation(
            summary = "Cancels a sessions in bulk (or individually too) by session ID",
            description = "Only admins/instructors/staff who is assigned to the class or session is authorized to perform this request."
    )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Success request - Request executed successfully"),
    ...
    public ResponseEntity<?> cancelSessionsBySessionIds(@RequestBody List<Integer> sessionIds) {
        try {
            log.info("executing method [sessionService].[cancelSessionsBySessionIds]");
            Response response = sessionService.cancelSessionsBySessionIds(sessionIds);
            log.info("[sessionService].[cancelSessionsBySessionIds] executed successfully");
            logger.logJsonObject("Response for [cancelSessionsBySessionIds]:\n{}", response);
            return ResponseEntity.ok().body(response);
        } catch (HandledRejection e) {
            return ResponseEntity
                    .badRequest()
    ...
    @PostMapping("/all/cancel-sessions-by-dates")
    @Operation(
            summary = "Cancels a sessions by its date",
            description = "Only admins/instructors/staff who is assigned to the class or session is authorized to perform this request."
    )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Success request - Request executed successfully"),
    ...
    public ResponseEntity<?> cancelSessionsByDates(@RequestBody List<LocalDate> dates) {
        try {
            log.info("executing method [sessionService].[cancelSessionsByDates]");
            Response response = sessionService.cancelSessionsByDates(dates);
            log.info("[sessionService].[cancelSessionsByDates] executed successfully");
            logger.logJsonObject("Response for [cancelSessionsByDates]:\n{}", response);
            return ResponseEntity.ok().body(response);
        } catch (HandledRejection e) {
            return ResponseEntity
                    .badRequest()
    ...
    @GetMapping("/all/get-sessions-dates")
    @Operation(
            summary = "Fetches dates of sessions of an active semester",
            description = "Only staff are authorized to perform this request."
    )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Success request - Request executed successfully"),
    ...
    public ResponseEntity<?> getDatesOfSessions() {
        try {
            log.info("executing method [sessionService].[getDatesOfSessions]");
            List<LocalDate> response = sessionService.getDatesOfSessions();
            log.info("[sessionService].[getDatesOfSessions] executed successfully");
            logger.logJsonObject("Response for [getSessionsByActiveSemester]:\n{}", response);
            return ResponseEntity.ok().body(response);
        } catch (HandledRejection e) {
            return ResponseEntity
    ...
    @GetMapping("/all/get-attendance-status")
    @Operation(
            summary = "This fetches if the attendance have been taken and the data needed depending on if the attendance have been taken",
    ...
    public ResponseEntity<?> getAttendanceStatus(@RequestParam int sessionId, @RequestParam int classId) {
        try {
            log.info("executing method [attendanceService].[getAttendanceStatus]");
            GetAttendanceStatusDto response = attendanceService.getAttendanceStatus(sessionId, classId);
            log.info("[attendanceService].[getAttendanceStatus] executed successfully");
            logger.logJsonObject("Response for [getAttendanceStatus]:\n{}", response);
            return ResponseEntity.ok().body(response);
        } catch (HandledRejection e) {
            return ResponseEntity
                    .badRequest()
