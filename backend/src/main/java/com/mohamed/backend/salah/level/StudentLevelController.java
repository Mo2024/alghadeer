...
public class StudentLevelController {
    ...
    @GetMapping("/all/level-by-student-id")
    @Operation(
            summary = "Gets student level by student id",
            description = "Only staff are authorized to perform this request."
    )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Success request - Request executed successfully"),
    ...
    public ResponseEntity<?> getStudentLevelObjectByStudentId(@RequestParam Integer studentId) {
        try {
            log.info("executing method [studentLevelService].[getStudentLevelObjectByStudentId]");
            StudentLevel response = studentLevelService.getStudentLevelObjectByStudentId(studentId);
            log.info("[studentLevelService].[getStudentLevelObjectByStudentId] executed successfully");
            logger.logJsonObject("Response for [getStudentLevelObjectByStudentId]:\n{}", response);
            return ResponseEntity.ok().body(response);
        } catch (HandledRejection e) {
            return ResponseEntity
                    .badRequest()
