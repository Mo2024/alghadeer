...
public class SubjectController {
    ...
    @GetMapping("/all/subjects-by-level")
    @Operation(
            summary = "Gets subjects by level of student",
    ...
    public ResponseEntity<?> getSubjectsByLevel(@RequestParam Integer studentId) {
        try {
            log.info("executing method [subjectService].[getSubjectsByLevel]");
            List<Subject> response = subjectService.getSubjectsByLevel(studentId);
            log.info("[subjectService].[getSubjectsByLevel] executed successfully");
            logger.logJsonObject("Response for [getSubjectsByLevel]:\n{}", response);
            return ResponseEntity.ok().body(response);
        } catch (HandledRejection e) {
            return ResponseEntity
                    .badRequest()
