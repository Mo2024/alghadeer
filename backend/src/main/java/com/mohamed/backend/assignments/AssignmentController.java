...
public class AssignmentController {
    ...
    private final AssignmentRepository.StudentAssignmentService studentAssignmentService;
    ...
    @PutMapping("/all/submit-assignment")
    @Operation(
            summary = "Submits assignment for a student",
            description = "Only instructors assigned to the class, supervisor and admin are authorized"
    )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Success request - Request executed successfully"),
    ...
    public ResponseEntity<?> submitAssignment(@RequestBody StudentsAssignment studentsAssignment) {
        try {
            log.info("executing method [studentAssignmentService].[submitAssignment]");
