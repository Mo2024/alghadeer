...
public class ClassController {
    ...
    @GetMapping("/supervisor/active-classes-with-sessions")
    @Operation(
            summary = "Fetches classes from by active semesters with sessions, it is similar to an API call above however this is with sessions",
            description = "Only admins and supervisor are authorized to perform this request."
    )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Success request - Request executed successfully"),
    ...
    public ResponseEntity<?> getActiveClassesWithSessions() {
        try {
            log.info("executing method [classService].[getActiveClassesWithSessions]");
            List<ClassView2> response = classService.getActiveClassesWithSessions();
            log.info("[classService].[getActiveClassesWithSessions] executed successfully");
            logger.logJsonObject("Response for [getActiveClassesWithSessions]:\n{}", response);
            return ResponseEntity.ok().body(response);
        } catch (HandledRejection e) {
            return ResponseEntity
                    .badRequest()
